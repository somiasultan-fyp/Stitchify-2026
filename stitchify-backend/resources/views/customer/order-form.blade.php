...
.form-control, .form-select {
  ...
  padding:12px 15px; color:var(--primary-bg);
  background-color:#f8f9fa; transition:all 0.3s ease; font-size:15px;
}
...
.pay-locked-block {
  ...
  padding:20px; text-align:center; background:#f8f9fa; margin-top:6px;
}
...
.info-block {
  ...
  background:#E6F1FB; border:1.5px solid #B5D4F4; margin-top:6px;
}
...
.btn-custom {
  ...
  padding:14px; font-weight:600; font-size:16px; width:100%;
...
.appt-toggle-box {
  ...
  background:#f8f9fa; padding:16px 18px; margin-bottom:4px;
}
.appt-toggle-label {
  font-size:14px; font-weight:600; color:var(--primary-bg);
  margin-bottom:12px; display:block;
}
.appt-options { display:flex; gap:10px; }
.appt-option {
  flex:1; display:flex; align-items:center; gap:8px;
  border:2px solid #e0e0e0; border-radius:10px;
  padding:10px 14px; cursor:pointer; background:#fff;
  transition:all 0.2s ease; font-size:14px; font-weight:500;
  color:var(--primary-bg); user-select:none;
}
...
.check-circle {
  ...
  justify-content:center; flex-shrink:0; transition:all 0.2s; background:#fff;
}
...
.appt-booking-section .appt-title {
  font-size:13px; font-weight:700; color:#3B6D11;
...
<div class="registration-wrapper">
  <div class="logo-container">
    <img src="{{ asset('logo.png') }}" alt="Stitchify" class="logo-image" onerror="this.style.display='none'">
    <h2>Stitchify</h2>
  ...
  <div class="form-body">
    ...
    <div class="info-block mb-3">
      <p><strong>Payment details</strong> will be available after order accepted. <strong>JazzCash / Easypaisa</strong> options are available.</p>
    </div>
...
<script>
  ...
  // CSRF token meta tag se lo
  const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

  function selectAppt(val) {
    apptChoice = val;
    document.getElementById('apptChoiceErr').textContent = '';
    document.getElementById('btnYes').className = 'appt-option' + (val === 'yes' ? ' selected' : '');
    document.getElementById('btnNo').className  = 'appt-option' + (val === 'no'  ? ' selected' : '');
    document.getElementById('apptBookingSection').style.display = val === 'yes' ? 'block' : 'none';
    document.getElementById('manualMeasSection').style.display  = val === 'no'  ? 'block' : 'none';
  }
    ...
    uploadedFiles.forEach((file, index) => {
      ...
      reader.onload = e => {
        ...
        wrap.innerHTML = `
          ...
          <button onclick="removeImage(${index})" style="position:absolute;top:-6px;right:-6px;width:20px;height:20px;border-radius:50%;background:#dc3545;color:#fff;border:none;cursor:pointer;font-size:11px;display:flex;align-items:center;justify-content:center;padding:0;">&#x2715;</button>
        `;
        previewBox.appendChild(wrap);
      };
      reader.readAsDataURL(file);
    });
    document.getElementById('imageLabel').textContent = uploadedFiles.length > 0
  ...
  function validate() {
    ...
    if (!apptChoice) {
      document.getElementById('apptChoiceErr').textContent = 'Please choose at least 1 option';
      ok = false;
  ...
  document.getElementById('submitBtn').addEventListener('click', async () => {
    ...
    formData.append('phone',                document.getElementById('cphone').value.trim());
    formData.append('address',              document.getElementById('caddr').value.trim());
    formData.append('city',                 document.getElementById('ccity').value.trim());
    ...
    formData.append('measurement_method',   apptChoice === 'yes' ? 'appointment' : 'manual');
    formData.append('_token',               csrfToken);

    if (apptChoice === 'yes') {
      formData.append('appointment_date', document.getElementById('apptDate').value);
      formData.append('appointment_time', document.getElementById('apptTime').value);
    } else {
      formData.append('chest',    document.getElementById('mChest').value    || '');
      formData.append('waist',    document.getElementById('mWaist').value    || '');
      formData.append('length',   document.getElementById('mLength').value   || '');
      formData.append('shoulder', document.getElementById('mShoulder').value || '');
      formData.append('sleeve',   document.getElementById('mSleeve').value   || '');
      formData.append('neck',     document.getElementById('mNeck').value     || '');
    }

    uploadedFiles.forEach(file => formData.append('design_images[]', file));
      ...
      if (result.success) {
        document.querySelector('.form-body').innerHTML = `
          <div style="text-align:center; padding:30px 10px;">
            ...
            <p style="color:#575a5b;font-size:14px;margin-bottom:20px;">Your order has been sent to the tailor.</p>
            <div style="background:#f8f9fa;border-radius:12px;padding:16px;text-align:left;border:2px solid #e0e0e0;margin-bottom:14px;">
              <p style="font-size:13px;color:#212529;margin-bottom:6px;"><strong>Order Number:</strong> ${result.order_number}</p>
              <p style="font-size:13px;color:#212529;margin-bottom:0;"><strong>Status:</strong> Pending</p>
            </div>
            <div style="background:#E6F1FB;border-radius:10px;padding:12px;border:1.5px solid #B5D4F4;margin-bottom:20px;">
              <p style="font-size:13px;color:#0C447C;margin:0;">
                ...
                <strong>Payment Pending</strong> — Waiting for tailor to accept your order.
              </p>
            </div>
            <button onclick="location.reload()" style="background:linear-gradient(135deg,#1B2A4A,#212529);color:#fff;border:none;border-radius:10px;padding:12px 30px;font-size:15px;font-weight:600;cursor:pointer;">
